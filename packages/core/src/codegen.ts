...
function generatedDtsPath(outputFile: string): string {
  return outputFile.endsWith(".ts")
    ? outputFile.replace(/\.ts$/, ".env.d.ts")
    : `${outputFile}.d.ts`;
}
...
function generateSinglePromptFile(
  ...
  const buildFnImports = partialInfos.map(
    ({ partial, buildFnName: bfn }) =>
      `import { ${bfn} } from "${generatedRelativeModule(generatedFile, generatedFilePath(partial))}";`
  );
  const typeImports = exposedPartials.map(
    ({ partial, pascalName: pn }) =>
      `import type { ${pn}Variables } from "${generatedRelativeModule(generatedFile, generatedFilePath(partial))}";`
  );
...
function generateCollisionFile(
  ...
  templates.forEach((t, i) => {
    lines.push(
      `import TEMPLATE_${i + 1} from "${relativeImport(dirname(generatedFile), t.filePath)}" with { type: "text" };`
    );
  });
...
function generateIndexFile(
  ...
  for (const { template } of groups) {
    lines.push(`export * from "${generatedRelativeModule(outputFile, generatedFilePath(template))}";`);
  }
  ...
  for (const { name, template } of groups) {
    ...
    lines.push(`import { ${buildFnName} } from "${generatedRelativeModule(outputFile, generatedFilePath(template))}";`);
  }
  ...
  lines.push(`type PromptArgs<Name extends PromptName> = Parameters<(typeof prompts)[Name]>;`);
  lines.push(``);
  ...
  lines.push(`): string {`);
  lines.push(`  return (prompts[name] as (...args: PromptArgs<Name>) => string)(...args);`);
  lines.push(`}`);
  lines.push(``);
  ...
  for (const [name, group] of grouped) {
    ...
    if (group.length === 1) {
      out.set(
        ...
        generateSinglePromptFile(firstTemplate, allTemplates, rootDir, generatedFilePath(firstTemplate))
      );
    } else {
      for (const template of group) {
        out.set(
          generatedFilePath(template),
          generateCollisionFile(group, allTemplates, rootDir, generatedFilePath(template))
        );
      }
    }
  }

  out.set(outputFile, generateIndexFile(manifestGroups, outputFile));
  out.set(generatedDtsPath(outputFile), generateTplDts());
